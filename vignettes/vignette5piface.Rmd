...
configFile="/Users/mstolarczyk/Desktop/testing/bedstat/test_bedstat.yaml"
...
The `outputs` section in the pipeline interface file and `outputsByPipeline` or `outputsByProtocol` functions provide a convenient access to the list of output file paths that are to be produced. 
...
```{r, echo=F, message=F}
knitr::kable(sampleTable(p),format = "html")
```
...
Similarly, the output file paths can be determined for a given protocol or set of protocols, like:
...
This functionality provides a convenient way to process the files produced by the pipeline, when used in the data processing function indicated in the `bioconductor` section of the pipeline interface file. See the example function below that demonstrates the application of the `outputsByPipeline` function.
