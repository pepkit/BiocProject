...
<dl>
  ...
  <dd>Please note, that the "getters and setters" methods associated with the `SummarizedExperiment` class (`rowRanges()`, `rowData()`, `colData()` etc.) as well as ones associated with the `Project` class (`samples()`, `config()` etc.) are inherited and work with the `BiocProject` class objects.</dd>
</dl>
