...
library(BiocProject)
```
Let's create an `BiocProject` object with just the one required argument (PEP config file) and see how it looks like
```{r}
bp = BiocProject()
bp
slotNames(bp)
...
<dl>
  ...
  <dd>Please note, that the "getters and setters" methods associated with the `SummarizedExperiment` class (`rowRanges()`, `rowData()`, `colData()` etc.) as well as ones associate with the `Project` class (`samples()`, `config()` etc.) are inherited and work with the `BiocProject` class objects.</dd>
</dl>
