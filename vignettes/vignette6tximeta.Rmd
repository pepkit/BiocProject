...
As you can see, this PEP configuration file uses a `$TXIMPORTDATA` environment variable to specify a file path. This is just an optional way to make this PEP work in any computing environment without being changed, so you can share your sample metadata more easily. (`tximportData` is a Bioconductor data package with various transcript quantification output, for unit testing of Bioconductor software packages.) For this vignette, we need to set the variable to the output directory where our downloaded results are stored:
