---
title: "Using a BiocProject with tximeta"
author: "Michal Stolarczyk"
date: "`r Sys.Date()`"
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Using a BiocProject with tximeta"}
...
As you can see in the PEP configuration file above `$TXIMPORT_DATA` environment variable has been used to make the path portable. Therefore, we need to set it. A reduced version of tximportData package contents is shipped with BiocProject, so let's use this sample dataset:
...
Even though the required `files` column is still missing this file is sufficient, since BiocProject, or more specifially pepr, will take care of construvting the portable `files` sample attribute automatically via `sample_modifiers.derive`:
...
```
After succesful import we can browse the resulting object. Since it is a `RangedSummarizedExperiment`, the methods defined in SummarizedExperiment package work:
```{r}
suppressPackageStartupMessages(library(SummarizedExperiment))
colData(bp)
assayNames(bp)
...
retrieveDb(bp)
```
And finally, note that the `PEP` metadata information has been attached to the metadata as well. Let's extract the `Project` object from the result:
