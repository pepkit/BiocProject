...
.getOutputs = function(pipeline, 
    ...
    if (!pepr::.checkSection(outputSchema, 
        sect)) {
    ...
    c(project = "Project"), function(project, 
        ...
        for (sampleName in defSampleNames) {
            ...
            for (pifaceSource in pifaceSources) {
                ...
                sampleRet[[pifaceSource]] = .populateTemplates(project, 
                  outputs, sampleName)
            }
    ...
    c(project = "Project"), function(project) {
        ...
        for (pifaceSource in pifaceSources) {
            ...
            outputs = .getOutputs(piface, 
                parent = dirname(pifaceSource), 
                projectContext = TRUE)
            ret[[pifaceSource]] = .populateTemplates(project, 
                outputs, projectContext = TRUE)
        }
        ret
    })
...
setMethod(".gatherSamplePipelineInterfaces", 
    c(project = "Project"), function(project) {
        ...
        .mkAbs = pryr::partial(.mkPathsAbs, 
            parent = dirname(project@file))
        if (PIP_IFACE_NAME %in% 
            colnames(t)) 
            return(setNames(vapply(unique(unlist(t[, 
                PIP_IFACE_NAME])), 
                .mkAbs, character(1)), 
                NULL))
        return(invisible(NULL))
...
.populateTemplates = function(project, 
    ...
    expandedTemplList = lapply(templList, 
        pepr::.expandPath)
    lapply(expandedTemplList, .populateString, 
        project, sampleName, projectContext)
}
